...
def run_experiment():
    ...
    Simulates 40 games consisting of 50 LLM-generated spins without
    the one-armed bandit role, and records the results.
    ...
    exp_name = "I_N"
    prompt_file = None
    results_file = "isolated_contextless"
    games_per_prompt = 40
    ...
    try:
        ...
        for i in range(games_per_prompt):
            ...
            while current_game["spin_number"] < spins_in_game:
                # Build prompt
                role = None
                task = "Your task is to provide three random and independent integers between 1 and 10, both inclusive, with equal probability. The integers must be separated by spaces."
                whole_prompt = f"{task}"
