...
def run_experiment():
    """
    Run experiment F_R_SP for all personas defined in personas.txt.

    Simulates 40 LLM-generated full games per persona with an intended length of 
    50 spins with the one-armed bandit role and implicit stats, and records the games.
    The full games are intended to be processed with the convert_full_to_spins.py
    script to resemble isolated-spin experiments. Each persona must be on its own
    line in the format "{persona_num} Persona description...".
    ...
    results_file = "full_role_persona"
    games_per_prompt = 40
    ...
    try:
        ...
        for prompt in prompts:
            ...
            for i in range(games_per_prompt):
                ...
                task = "Your task is to simulate a game. The player stops playing after 50 rounds. Write each spin result on a separate line and nothing else. The reel numbers of the spins must be separated by spaces."
                ...
                results.append({
                    ...
                    "exp_name": f"{exp_name}{persona_num}",
